...
func getConnectString(conf *ClickhouseConfig, db bool) string {
	...
	if db {
		return fmt.Sprintf("tcp://%s:%s?username=%s&password=%s&database=%s", conf.Host, conf.Port, conf.User, conf.Password, conf.DbName)
	}

	return fmt.Sprintf("tcp://%s:%s?username=%s&password=%s", conf.Host, conf.Port, conf.User, conf.Password)
}
